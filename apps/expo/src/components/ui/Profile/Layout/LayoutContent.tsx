...
export const PROFILE_SCREEN_HEADER_HEIGHT = 80;
export const LayoutContent = ({ children }: PropsWithChildren) => {
  const { top, height } = useHeaderMeasurements();
  const style = useAnimatedStyle(() => {
    return {
      top: top.value,
      marginTop: (height.value ?? PROFILE_SCREEN_HEADER_HEIGHT) + TABBAR_HEIGHT,
    };
  });
  const innerStyle = useAnimatedStyle(() => {
    return {
      top: -top.value,
      marginTop:
        -(height.value ?? PROFILE_SCREEN_HEADER_HEIGHT) - TABBAR_HEIGHT,
    };
  });
